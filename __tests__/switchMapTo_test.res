...
describe("switchMapTo", ({test, testAsync, beforeEach, afterEach}) => {
  ...
  test("tbd", ({expect}) => {
    ...
    Wonka.fromValue(1) |> WonkaExtras.switchMapTo(sourceA) |> Wonka.forEach(spy)
  ...
  testAsync("tbd2", ({expect, callback}) => {
    ...
    Wonka.fromPromise(promise200(1)) |> WonkaExtras.switchMapTo(sourceA) |> Wonka.forEach(spy)
